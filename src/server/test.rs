...
pub fn wait_rec_state_started(port: u16) {
    let check_rdy = || {
        let res = minreq::get(format!("http://127.0.0.1:{port}/edl-recording-state"))
            ...
            .json::<ResBody>()
            .unwrap();
        res.recording_state == EdlRecordingState::Started
    };
    while !check_rdy() {
...
fn edit_starts_cut_ends_diss() {
    ...
    assert_eq!(
        end_res.final_edits()[1].dissolve().from.av_channels,
        AVChannels::new(true, 2)
    );
    assert_eq!(
        end_res.final_edits()[1]
...
fn edit_starts_diss_ends_cut() {
    ...
    assert_eq!(wipe_1_res.rec_state(), EdlRecordingState::Started);
    assert_eq!(
        wipe_1_res.dissolve().from.source_tape.to_string(),
        "BL".to_string()
    );
    assert_eq!(
    ...
    assert_eq!(
        wipe_1_res.dissolve().to.source_tape.to_string(),
        "tape1".to_string()
    );
    assert_eq!(
    ...
    assert_eq!(
        cut_1_res.wipe().from.source_tape.to_string(),
        "tape1".to_string()
    );
    assert_eq!(cut_1_res.wipe().from.av_channels, AVChannels::new(false, 1));
    assert_eq!(
        cut_1_res.wipe().to.source_tape.to_string(),
        "tape2".to_string()
    );
...
fn select_src_and_alternate() {
    ...
    assert_eq!(
        cut_7_res.wipe().from.source_tape.to_string(),
        "tape2".to_string()
    );
    assert_eq!(
        cut_7_res.wipe().to.source_tape.to_string(),
        "tape1".to_string()
    );
    ...
    let cut_9_res = minreq::post(format!("http://127.0.0.1:{port}/log"))
        .with_header("Content-Type", "application/json")
        .with_body(serde_edit(EditRequestData {
            edit_type: "cut".into(),
            edit_duration_frames: None,
        ...
        .send()
        .unwrap();
    assert_eq!(cut_9_res.status_code, 200);
    assert_eq!(cut_9_res.rec_state(), EdlRecordingState::Started);
    assert_eq!(cut_9_res.cut().source_tape.to_string(), "tape2".to_string());

    let cut_10_res = minreq::post(format!("http://127.0.0.1:{port}/log"))
        .with_header("Content-Type", "application/json")
        .with_body(serde_edit(EditRequestData {
            edit_type: "cut".into(),
            edit_duration_frames: None,
        ...
        .send()
        .unwrap();
    assert_eq!(cut_10_res.status_code, 200);
    assert_eq!(cut_10_res.rec_state(), EdlRecordingState::Started);
    assert_eq!(
        cut_10_res.cut().source_tape.to_string(),
        "tape2".to_string()
    );
...
fn event_non_ready() {
    ...
    assert_eq!(
        end_before_start.final_edits()[0]
            .cut()
            .source_tape
            .to_string(),
        "BL".to_string()
